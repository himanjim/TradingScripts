...
scalper_responses_excel_location = 'F:/Trading_Responses/ScalperPriceChange_excel_backtest' + str (
    current_time) + '.xlsx'
...
max_price_rise = {nse_bse.STOCK_ID: None, PRICE_DIFF: -math.inf}
max_price_fall = {nse_bse.STOCK_ID: None, PRICE_DIFF: math.inf}
        ...
        for i in range (len (stock_data)):
            ...
            if stock_time.time () == MARKET_START_TIME:
                if open < close:
                    ...
                    key = str (stock_time) + 'high'
                    if key not in price_changes or price_change > price_changes[key][PRICE_DIFF]:
                        price_changes[key] = {nse_bse.STOCK_ID: stock_id, PRICE_DIFF: price_change}
...
for scalper_response in scalper_responses:
    ...
    if (key_high in price_changes and scalper_response[0] == price_changes[key_high][nse_bse.STOCK_ID]) or (
            key_low in price_changes and scalper_response[0] == price_changes[key_low][nse_bse.STOCK_ID]):
        scalper_response.append (1)
    else:
        scalper_response.append (0)
