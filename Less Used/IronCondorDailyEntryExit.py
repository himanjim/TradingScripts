...
def get_trading_outputs(df_part_, under_lying_value_rnd_, second_trade_, trading_days_diff_):
    ...
    if second_trade_ is False:
        sell_put_entry_price_ = float(
            df_part_.loc[(df_part_['Strike Price  '] == sell_put_strike_) & (df_part_['Option type  '] == 'PE')][
                'Open  '].iloc[0])
        sell_call_entry_price_ = float(
            df_part_.loc[(df_part_['Strike Price  '] == sell_call_strike_) & (df_part_['Option type  '] == 'CE')][
                'Open  '].iloc[0])
    else:
    ...
    # ****************
    trading_date_obj_ = df_part_.iloc[0]['date_obj']
    new_date_time_obj = datetime(year=trading_date_obj_.year, month=trading_date_obj_.month, day=trading_date_obj_.day, hour=10, minute=0)
    df_bn_5min_part_ = df_bn_5min.loc[(df_bn_5min['Date'] == new_date_time_obj)]

    if df_bn_5min_part_.shape[0] != 0:
        df_bn_5min_open = df_bn_5min_part_['Open'].iloc[0]

        sell_call_strike_ = sell_put_strike_ = round(df_bn_5min_open / 100) * 100
    # ****************

    sell_put_exit_price_ = float(
        df_part_.loc[(df_part_['Strike Price  '] == sell_put_strike_) & (df_part_['Option type  '] == 'PE')][
            'Close  '].iloc[0])
    sell_call_exit_price_ = float(
        df_part_.loc[(df_part_['Strike Price  '] == sell_call_strike_) & (df_part_['Option type  '] == 'CE')][
            'Close  '].iloc[0])

    p_l_ = (sell_put_entry_price_ + sell_call_entry_price_) - (sell_put_exit_price_ + sell_call_exit_price_)

    maximum_ce_short_premium_ = float(
        df_part_.loc[(df_part_['Strike Price  '] == sell_call_strike_) & (df_part_['Option type  '] == 'CE')][
            'High  '].iloc[0])
    minimum_ce_short_premium_ = float(
        df_part_.loc[(df_part_['Strike Price  '] == sell_call_strike_) & (df_part_['Option type  '] == 'CE')][
            'Low  '].iloc[0])

    maximum_pe_short_premium_ = float(
        df_part_.loc[(df_part_['Strike Price  '] == sell_put_strike_) & (df_part_['Option type  '] == 'PE')][
            'High  '].iloc[0])
    minimum_pe_short_premium_ = float(
        df_part_.loc[(df_part_['Strike Price  '] == sell_put_strike_) & (df_part_['Option type  '] == 'PE')][
            'Low  '].iloc[0])

    maximum_loss_ = (
                sell_call_entry_price_ + sell_put_entry_price_ - maximum_ce_short_premium_ - minimum_pe_short_premium_) if maximum_ce_short_premium_ >= maximum_pe_short_premium_ else (
...
def get_concatenated_df(files_pattern_):
    # Get a list of all CSV files matching the pattern
    csv_files_ = glob.glob(files_pattern_)

    with ThreadPoolExecutor() as executor:
        dfs_ = list(executor.map(read_csv, csv_files_))

    # Concatenate all DataFrames into a single DataFrame
    return pd.concat(dfs_, ignore_index=True)


def string_to_datetime(datetime_str_):
    datetime_str_ = datetime_str_.replace(' GMT+0530 (India Standard Time)', '')

    # Parse the string into a datetime object
    return datetime.strptime(datetime_str_, '%a %b %d %Y %H:%M:%S')
...
if __name__ == '__main__':
    ...
    underlying_file = DRIVE + '/NIFTY BANK_Historical_PR_01042017to28062024.csv'
    UNDERLYING = 'BN'
    ...
    # Concatenate all DataFrames into a single DataFrame
    df = get_concatenated_df(files_pattern)

    # **********************
    df_bn_5min = pd.read_pickle('D:/BN 5 min/BN_5_min.pkl')

    # df_bn_5min = get_concatenated_df(DRIVE + '/BN 5min data/*.csv')
    # df_bn_5min['Date'] = df_bn_5min['Date'].apply(string_to_datetime)
    # **********************
    ...
    for trading_date in all_dates:
        ...
        if expiry_date_obj is not None:
            ...
            try:
                ...
                sell_put_strike, sell_put_entry_price, sell_put_exit_price, sell_call_strike, sell_call_entry_price, sell_call_exit_price, p_l, managed_profit, max_short_premium_ce, min_short_premium_ce, max_short_premium_pe, min_short_premium_pe, max_premium_type, maximum_beareable_loss, maximum_loss = get_trading_outputs(
                    df_part, under_lying_value_rnd, False, trading_days_diff)
            ...
            trading_outputs.append(
                [UNDERLYING, under_lying_value, trading_date_obj, expiry_date_obj, trading_days_diff, sell_put_strike,
                 sell_put_entry_price, sell_put_exit_price, sell_call_strike, sell_call_entry_price,
                 sell_call_exit_price, p_l, 1 if p_l > 0 else 0, managed_profit, 1 if managed_profit > 0 else 0,
                 max_short_premium_ce, min_short_premium_ce, max_short_premium_pe, min_short_premium_pe,
                 max_premium_type, maximum_loss, maximum_beareable_loss, second_trade_underlying,
                 second_trade_call_open_price, second_trade_call_exit_price, second_trade_put_open_price,
                 second_trade_put_exit_price, second_trade_p_l,
                 1 if (second_trade_p_l != None and second_trade_p_l > 0) else 0, second_trade_managed_profit,
                 1 if (second_trade_managed_profit is not None and second_trade_managed_profit > 0) else 0,
                 second_trade_max_short_premium_ce, second_trade_min_short_premium_ce,
                 second_trade_max_short_premium_pe, second_trade_min_short_premium_pe, second_trade_max_premium_type,
                 second_trade_maximum_loss])

    if len(trading_outputs) > 0:
        trading_outputs = sorted(trading_outputs, key=lambda x: x[3])
        trading_outputs.insert(0, ['UNDERLYING', 'VALUE', 'TRADE DATE', 'EXPIRY DATE', 'DIFF. DAYS', 'SELL PUT',
                                   'SELL PUT(EN)', 'SELL PUT(EX)', 'SELL CALL', 'SELL CALL(EN)', 'SELL CALL(EX)', 'P/L',
                                   'PROFIT', 'MGD PROFIT', 'MGD P/L', 'MAX PREM CE', 'MIN PREM CE', 'MAX PREM PE',
                                   'MIN PREM PE', 'MAX PREM TYPE', 'MAX LOSS', 'MAX BEAR, LOSS', '2ND TRADE UNDERLYING',
                                   '2ND TRADE CALL OPEN', '2ND TRADE CALL EX', '2ND TRADE PUT OPEN', '2ND TRADE PUT EX',
                                   '2ND TRADE PL', '2ND TRADE P/L', '2ND TRADE MG PROFIT', '2ND TRADE MG P/L',
                                   '2ND TRADE MAX PREM CE', '2ND TRADE MIN PREM CE', '2ND TRADE MAX PREM PE',
                                   '2ND TRADE MIN PREM PE', '2ND TRADE PREM TYPE', '2ND MAX LOSS'])
        ...
        excel_df.to_excel(DRIVE + "".join(("/", str(UNDERLYING), '_', str(STRIKE_DIFF_PERCENT),
                                           str(maximum_beareable_loss), '_IronCondor_Same_Day_2_attempts.xlsx')),
                          index=False)
