#!/usr/bin/env python3
"""
Zerodha Live Liquid Stock Grid
==============================

Purpose
-------
This Dash application does the following:

1. Reads a CSV containing one column named ``symbol``.
2. Matches those symbols with Zerodha/Kite NSE equity instruments.
3. Scans live quotes and selects the 50 stocks with the highest live traded volume.
4. Backfills today's missing 1-minute candles for those 50 stocks using Kite historical_data().
5. Fetches previous trading day's daily candle and computes CPR/pivot levels.
6. Displays all 50 live charts in a scrollable 5 x 10 grid.
7. Refreshes charts every second by polling all 50 instruments in one kite.quote([...]) call.

Why polling is used here
------------------------
The user specifically requested kite.quote polling. WebSocket ticks are usually better for
sub-second market data, but this script deliberately uses quote polling so that all 50
selected instruments are refreshed together once per second.
...
Symbols may also be written as NSE:RELIANCE. The exchange prefix will be stripped.
...
This is the same initialization pattern used in your existing scripts.

Example
-------
    python zerodha_live_liquid_stock_grid.py --symbols-csv nse_symbols.csv

Optional examples
-----------------
    python zerodha_live_liquid_stock_grid.py --symbols-csv nse_symbols.csv --top-n 50 --port 8050
    python zerodha_live_liquid_stock_grid.py --symbols-csv nse_symbols.csv --refresh-ms 1000
    python zerodha_live_liquid_stock_grid.py --symbols-csv nse_symbols.csv --cache-dir ./live_stock_cache

Important practical note
------------------------
Kite's historical API is not bulk. Startup backfill is therefore one historical request per
selected stock, plus one daily historical request per selected stock for pivots. The code
throttles these requests to reduce the probability of 429/rate-limit errors.
"""
...
# Quote API accepts 500 instruments in one quote call. The live loop uses only
# top_n=50, but startup scanning may have more symbols, so it is chunked.
...
# Mini-chart sizing. With 5 columns, 240-280px is usually usable on a desktop.
DEFAULT_CHART_HEIGHT_PX = 260
...
# Number of intraday candles to show in each mini chart. Use 390 to show the full
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Live Zerodha dashboard for top-volume NSE stocks in a 5x10 Plotly/Dash grid."
    )
    parser.add_argument(
        "--symbols-csv",
        default=r"C:\Users\Local User\Downloads\fo_mktlots.csv",
        help="CSV file containing a column named 'symbol'.",
    )
    parser.add_argument(
        "--exchange",
        default="NSE",
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of most-liquid stocks to display. Default: 50.",
    )
    parser.add_argument(
        "--refresh-ms",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Height of each mini chart in pixels. Default: {DEFAULT_CHART_HEIGHT_PX}.",
    )
    parser.add_argument(
...
def poll_top50_once(kite: "KiteConnect", selected: Sequence[StockMeta], state: LiveState) -> Tuple[int, Optional[str]]:
    """Poll all selected instruments in one quote() call and update live state."""
...
def make_app(
    ...
    """Create the Dash app containing a 5 x 10 scrollable chart grid.
    ...
    """
    app = Dash(__name__)
    app.title = "Zerodha Top 50 Live Stock Grid"
        ...
        [
            ...
            html.Div(
                [
                    html.Div(
                        [
                            html.Div("Zerodha Top 50 Live-Volume Stock Grid", className="main-title"),
                            html.Div(
                                (
                                    f"{len(selected)} charts | refresh={refresh_ms} ms | "
                                    "one kite.quote() call per refresh after startup | "
    ...
    def refresh_dashboard(n_intervals: int):
        ...
        """
        try:
            updated, quote_error = poll_top50_once(kite, selected, state)
            ...
            if quote_error:
                ...
                status = (
                    ...
                    f"charts with candles: {non_empty}/{len(selected)} | selected by startup live volume"
                )
...
def main() -> None:
    ...
    print("[CONFIG] symbols_csv       =", args.symbols_csv)
    print("[CONFIG] exchange          =", args.exchange)
    print("[CONFIG] top_n             =", args.top_n)
    ...
    if args.top_n > 50:
        print(
            "[WARN] You asked for more than 50 charts. The script can do it, but the browser may become heavy. "
            "The original requirement was 50."
        )
    ...
    csv_symbols = load_symbols_from_csv(args.symbols_csv)
    print(f"[STEP] CSV symbols loaded: {len(csv_symbols):,}")

    equity_map = build_nse_equity_map(kite, args.exchange)
    matched = match_csv_symbols_to_kite(csv_symbols, equity_map, args.exchange)
    print(f"[STEP] CSV symbols matched with Kite NSE EQ instruments: {len(matched):,}")

    selected = select_top_liquid_stocks(kite, matched, args.exchange, args.top_n)
    if not selected:
        raise RuntimeError("No selected stocks after live-volume scan.")
    ...
    print("\nLive loop: one kite.quote([...top 50 keys...]) call per refresh.")
