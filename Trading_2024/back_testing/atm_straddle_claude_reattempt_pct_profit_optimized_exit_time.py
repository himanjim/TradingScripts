...
def _params_from_trial(trial, base: "Params") -> "Params":
    ...
    # Base point is 09:20 because 09:15 option data can be noisy/incomplete.
    ...
    # EXIT_TIME_IST: absolute last-entry cutoff on a 5-minute grid from 09:20
    # to 15:30. If Optuna suggests a value before the entry time, clamp it up to
    # the entry time so the config is still a valid "entry <= exit cutoff" setup.
    # This keeps search compatible with Optuna's fixed-distribution requirement.
    exit_min = trial.suggest_int("exit_minute_from_0920", 0, 370, step=5)
...
def optimize(
    ...
    def _progress(study_, trial_):
        ...
        print(
            f"[TRIAL {n_done:>4}/{n_trials}] profit={_inr(ua.get('total_pnl', 0))} "
            ...
            f"| MAX PROFIT={_inr(best_val)} | {elapsed:5.0f}s eta={eta:6.0f}s",
            flush=True,
        )
