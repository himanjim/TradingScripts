...
# "straddle_config_dte0_profit_max.properties" next to this file;
...
BUILD_ID = "dte0-profit-target-reentry-v4.0-20260724"
...
# --- Per-attempt profit target as a fraction of collected premium -----------
# The legacy behaviour stops for the day after PROFIT_TARGET.  The optimized
# build can instead recycle the released capital into a fresh ATM straddle
# after a separately configured target delay.  Positions remain sequential:
# at most one CE+PE straddle is open, so QTY_UNITS/lots never increase.
PROFIT_TARGET_PCT = float(os.getenv("PROFIT_TARGET_PCT", 0.7))
PROFIT_TARGET_REENTRY_ENABLED = (
    os.getenv("PROFIT_TARGET_REENTRY_ENABLED", "0").strip() == "1"
)
PROFIT_TARGET_REENTRY_DELAY_BY_ATTEMPT = _parse_int_list(
    os.getenv("PROFIT_TARGET_REENTRY_DELAY_BY_ATTEMPT"),
    [3],
)
...
_DEFAULT_OUT = os.path.join(
    ...
    "short_straddle_dte0_profit_optimized.xlsx",
)
...
class UnderlyingStrategySettings:
    ...
    profit_target_reentry_delay_by_attempt: Tuple[int, ...]
    ...
    def target_reentry_delay_for_attempt(self, attempt_idx: int) -> int:
        values = self.profit_target_reentry_delay_by_attempt
        return int(values[min(attempt_idx, len(values) - 1)]) if values else 0
...
def _underlying_settings(prefix: str) -> UnderlyingStrategySettings:
    ...
    return UnderlyingStrategySettings(
        ...
        profit_target_reentry_enabled=(
            os.getenv(
                f"{prefix}_PROFIT_TARGET_REENTRY_ENABLED",
                "1" if PROFIT_TARGET_REENTRY_ENABLED else "0",
            ).strip()
            == "1"
        ),
        profit_target_reentry_delay_by_attempt=tuple(
            _parse_int_list(
                os.getenv(
                    f"{prefix}_PROFIT_TARGET_REENTRY_DELAY_BY_ATTEMPT"
                ),
                PROFIT_TARGET_REENTRY_DELAY_BY_ATTEMPT,
            )
        ),
...
def simulate_day_multi_trades(
    *,
    ...
    while cur_entry_ts < trade_end_ts:
        ...
        # --- Per-attempt PROFIT TARGET: % of collected premium ---------------
        # The exit trigger/fill convention is intentionally unchanged from the
        # supplied source engine.  Only the post-exit decision is configurable:
        # legacy profiles stop for the day, while the optimized profile may
        # re-enter after its target-specific delay.
        target_ts = None
        target_rupees = None
        ...
        target_reentry = bool(
            ...
            and settings.profit_target_reentry_enabled
        )
        risk_reentry = exit_reason in ("STOPLOSS", "PROFIT_PROTECT")
        if (
            (risk_reentry or target_reentry)
            and (trade_seq - 1) < settings.max_reattempts
        ):
            if target_reentry:
                delay_min = settings.target_reentry_delay_for_attempt(
                    trade_seq - 1
                )
            else:
...
def main():
    ...
    for _profile_name, _settings in (
        ...
        print(
            ...
            f"target re-entry {_settings.profit_target_reentry_enabled} | "
            f"target delays "
            f"{list(_settings.profit_target_reentry_delay_by_attempt)} | "
