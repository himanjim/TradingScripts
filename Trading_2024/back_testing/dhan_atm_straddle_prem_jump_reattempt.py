...
REENTRY_DELAY_MINUTES = int(os.getenv("REENTRY_DELAY_MINUTES", "60"))
...
def simulate_day_multi_trades_dhan(
    *,
    ...
    while cur_entry_ts <= session_end_ts:
        ...
        if exit_reason == "STOPLOSS" and exit_pnl < -LOSS_LIMIT_RUPEES:
            exit_pnl = -float(LOSS_LIMIT_RUPEES)
...
def main():
    ...
    print(f"[INFO] Stoploss: -{LOSS_LIMIT_RUPEES} | ProfitProtect giveback: {PROFIT_PROTECT_TRIGGER_RUPEES} | Re-entry delay: {REENTRY_DELAY_MINUTES}m")
    ...
    # Excel output
    write_excel(all_trades_df, actual_trades_df, daily_pnl_df, monthwise_df, skipped_df)
