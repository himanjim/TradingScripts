...
MAX_REATTEMPTS = int(os.getenv("MAX_REATTEMPTS", "8"))  # 8 = up to 8 re-entries after the first trade

# Re-entry rule:
# After a STOPLOSS / PROFIT_PROTECT exit, the script no longer waits for a fixed
# number of minutes. It waits until the underlying index itself shows a small
# reversal-confirmation pattern:
#   1) infer the trend direction from entry-underlying-price to exit-underlying-price;
#   2) find the latest 1-minute underlying candle in that trend direction;
#   3) wait for 2 consecutive 1-minute candles in the opposite direction;
#   4) require the combined high-low range of those 2 reversal candles to fully
#      cover the high-low range of the latest trend candle.
# Entry is taken at the close timestamp of the second confirming reversal candle.
REENTRY_REVERSAL_CANDLES = 2
...
_DEFAULT_OUT = os.path.join(
    ...
    f"_REV{REENTRY_REVERSAL_CANDLES}_FULLCOVER.xlsx"
)
...
class TradeRow:
    ...
    qty_units: int
    entry_time: str
    entry_trigger: str       # INITIAL_ENTRY_TIME or REVERSAL_FULL_COVER
    entry_signal_note: str   # human-readable explanation of why this entry was taken
    exit_time: str
    exit_reason: str
...
def _prepare_underlying_ohlc(underlying_day: pd.DataFrame, idx_all: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Build an indexed 1-minute OHLC dataframe for the underlying index.

    This is used only for the re-entry trigger. Unlike PnL tracking, OHLC
    candles are NOT forward-filled here, because a reversal signal must be
    formed by actual 1-minute candles. Missing candles invalidate the pattern.
    """
    needed = ["date", "open", "high", "low", "close"]
    missing = [c for c in needed if c not in underlying_day.columns]
    if missing:
        raise ValueError(f"Underlying dataframe missing OHLC columns required for re-entry pattern: {missing}")

    u = underlying_day[needed].dropna().copy()
    if u.empty:
        return pd.DataFrame(index=idx_all, columns=["open", "high", "low", "close"], dtype="float64")
    ...
    return u[["open", "high", "low", "close"]]


def _candle_direction(row: pd.Series) -> int:
    """
    Return candle direction: +1 bullish, -1 bearish, 0 doji/missing.

    The re-entry trigger deliberately ignores doji candles because they do not
    prove an opposite-direction reversal.
    """
    try:
        o = float(row["open"])
        c = float(row["close"])
...
def _infer_trend_direction(
    ...
    """
    Infer the trend that hurt the short straddle.

    Primary rule: compare underlying close at exit with underlying price at entry.
      +1 = uptrend, -1 = downtrend.

    Fallback: when exit close equals entry price or is missing, use the latest
    non-doji candle between entry and exit.
    """
    exit_close = float("nan")
    if exit_ts in underlying_ohlc.index and pd.notna(underlying_ohlc.loc[exit_ts, "close"]):
        exit_close = float(underlying_ohlc.loc[exit_ts, "close"])
    else:
        # Use last available close up to exit_ts if the exact exit candle is missing.
        prior = underlying_ohlc.loc[:exit_ts, "close"].dropna()
        if not prior.empty:
            exit_close = float(prior.iloc[-1])

    if pd.notna(exit_close) and pd.notna(entry_underlying):
        if exit_close > float(entry_underlying):
            return 1
        if exit_close < float(entry_underlying):
            return -1

    # Flat / missing fallback: latest decisive candle direction inside the trade.
    window = underlying_ohlc.loc[entry_ts:exit_ts].dropna(subset=["open", "close"])
    if not window.empty:
        for _, row in window.iloc[::-1].iterrows():
            d = _candle_direction(row)
            if d != 0:
                return d
...
def _find_last_trend_candle(
    ...
    """
    Find the latest underlying candle, between trade entry and exit, that moved
    in the trend direction. That candle becomes the reference candle to be
    fully covered by the 2 opposite-direction candles.
    """
    if trend_dir not in (1, -1):
        return None

    window = underlying_ohlc.loc[entry_ts:exit_ts].dropna(subset=["open", "high", "low", "close"])
    if window.empty:
        return None

    for ts, row in window.iloc[::-1].iterrows():
        if _candle_direction(row) == trend_dir:
            return pd.Timestamp(ts), row

    return None


def _find_reentry_after_two_candle_full_cover(
    underlying_ohlc: pd.DataFrame,
    entry_ts: pd.Timestamp,
    exit_ts: pd.Timestamp,
    entry_underlying: float,
    ...
    """
    Locate the next re-entry timestamp using the new reversal rule.

    Rule implemented:
      - Determine whether the failed trade was hurt by an uptrend or downtrend.
      - Identify the latest 1-minute candle in that trend direction.
      - Wait after the exit for exactly two consecutive 1-minute candles in the
        opposite direction.
      - The combined high-low range of those two candles must fully cover the
        high-low range of the latest trend candle.
      - Return the timestamp of the second reversal candle. The backtest enters
        there using that candle's close, consistent with the rest of this script.

    For an uptrend, the two confirmation candles must both be bearish.
    For a downtrend, they must both be bullish.
    """
    trend_dir = _infer_trend_direction(underlying_ohlc, entry_ts, exit_ts, entry_underlying)
    if trend_dir == 0:
        return None, "Cannot infer failed-trade trend direction from underlying candles"

    last_trend = _find_last_trend_candle(underlying_ohlc, entry_ts, exit_ts, trend_dir)
    if last_trend is None:
        return None, "No decisive latest trend candle found between entry and exit"

    last_trend_ts, last_trend_row = last_trend
    trend_high = float(last_trend_row["high"])
    trend_low = float(last_trend_row["low"])
    reverse_dir = -trend_dir
    reverse_label = "bearish" if reverse_dir == -1 else "bullish"

    # Start from the first full minute after exit. This avoids re-entering on
    # the same candle that triggered the exit.
    first_check_ts = pd.Timestamp(exit_ts) + pd.Timedelta(minutes=1)
    if first_check_ts > session_end_ts:
        return None, "Exit occurred too close to session end for re-entry confirmation"

    future = underlying_ohlc.loc[first_check_ts:session_end_ts]
    if len(future) < REENTRY_REVERSAL_CANDLES:
        return None, "Fewer than 2 underlying candles available after exit"

    # The loop is intentionally written for exactly two candles because the
    # strategy rule requested is specific: 2 one-minute opposite candles.
    for i in range(0, len(future) - 1):
        c1 = future.iloc[i]
        c2 = future.iloc[i + 1]
        ts1 = pd.Timestamp(future.index[i])
        ts2 = pd.Timestamp(future.index[i + 1])

        # Missing OHLC data means the pattern was not actually observable.
        if c1[["open", "high", "low", "close"]].isna().any() or c2[["open", "high", "low", "close"]].isna().any():
            continue

        if _candle_direction(c1) != reverse_dir or _candle_direction(c2) != reverse_dir:
            continue

        combined_high = max(float(c1["high"]), float(c2["high"]))
        combined_low = min(float(c1["low"]), float(c2["low"]))

        # Strict full-cover condition: combined range of the 2 reversal candles
        # must contain the complete range of the last trend candle.
        fully_covers = combined_high >= trend_high and combined_low <= trend_low
        if not fully_covers:
            continue

        trend_label = "UP" if trend_dir == 1 else "DOWN"
        reason = (
            f"Re-entry at {ts2.strftime('%H:%M')}: failed-trade trend={trend_label}; "
            f"last trend candle={last_trend_ts.strftime('%H:%M')} "
            f"range {trend_low:.2f}-{trend_high:.2f}; "
            f"two {reverse_label} candles {ts1.strftime('%H:%M')},{ts2.strftime('%H:%M')} "
            f"fully covered it with range {combined_low:.2f}-{combined_high:.2f}"
        )
        return ts2, reason
    ...
    return (
        None,
        f"No 2-candle opposite-direction full-cover re-entry signal after exit; "
        f"failed-trade trend={trend_label}, last trend candle={last_trend_ts.strftime('%H:%M')}"
    )
...
def simulate_day_multi_trades(
    *,
    ...
    # Underlying OHLC candles drive the new reversal-based re-entry trigger.
    # Option candles are still used for option entry/exit pricing and PnL.
    underlying_ohlc = _prepare_underlying_ohlc(underlying_day, idx_all)
    ...
    cur_entry_trigger = "INITIAL_ENTRY_TIME"
    cur_entry_signal_note = f"Scheduled first entry at configured ENTRY_TIME_IST={ENTRY_TIME_IST}"
    trade_seq = 1
    ...
    while cur_entry_ts <= session_end_ts:
        ...
        if pd.isna(ce_entry) or pd.isna(pe_entry):
            skipped.append({"day": dy, "underlying": und, "expiry": expiry, "trade_seq": trade_seq,
                            "atm_strike": atm, "reason": "No exact CE/PE close at entry timestamp"})
            break
        ...
        results.append(
            TradeRow(
                ...
                entry_trigger=cur_entry_trigger,
                entry_signal_note=cur_entry_signal_note,
        ...
        if exit_reason in ("STOPLOSS", "PROFIT_PROTECT") and (trade_seq - 1) < MAX_REATTEMPTS:
            # Re-entry is no longer time-delay based.
            # We wait for the underlying to print 2 consecutive 1-minute candles
            # opposite to the failed-trade trend, and those 2 candles must fully
            # cover the latest trend candle. This prevents blind re-entry during
            # a continuing one-way move.
            next_entry_ts, reentry_reason = _find_reentry_after_two_candle_full_cover(
                underlying_ohlc=underlying_ohlc,
                entry_ts=pd.Timestamp(cur_entry_ts),
            ...
            trade_seq += 1
            cur_entry_ts = pd.Timestamp(next_entry_ts)
            cur_entry_trigger = "REVERSAL_FULL_COVER"
            cur_entry_signal_note = reentry_reason
...
def main():
    ...
    print(
        f"[INFO] Stoploss: -{LOSS_LIMIT_RUPEES} | "
        f"ProfitProtect giveback: {PROFIT_PROTECT_TRIGGER_RUPEES} | "
        f"Re-entry: {REENTRY_REVERSAL_CANDLES} opposite 1-min candles fully covering latest trend candle"
    )
