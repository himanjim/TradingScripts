"""ATM short-straddle backtest -- v3.

v3 ADDITIONS (all opt-in via the .properties file; leaving the new keys unset
reproduces v2 behaviour exactly):

  A. REENTRY_ON_PROFIT_TARGET=1
     A PROFIT_TARGET exit no longer ends the day: the target is banked and the
     strategy re-enters at the new ATM after REENTRY_DELAY_AFTER_TARGET
     minutes. This is the single biggest change: on 2022-07..2026-07 DTE-1
     data the actual_trades net P&L went from Rs.14.77L to Rs.42.5L on the
     same fill basis, with a SMALLER max drawdown (-25k vs -38k), and it
     improved all 6 independent time blocks and every calendar year.

  B. REENTRY_DELAY_AFTER_TARGET (default: falls back to REENTRY_DELAY_BY_ATTEMPT)
     Separate re-entry delay after target exits. 1 minute tested best:
     nothing adverse happened at a target exit, so there is nothing to wait out.

  C. DAILY_PROFIT_TRAIL_ARM_RUPEES / DAILY_PROFIT_TRAIL_GIVEBACK_RUPEES
     Day-level profit trail: once cumulative realized day P&L reaches ARM, the
     day stops as soon as realized P&L falls GIVEBACK below the day's realized
     peak. 10000/10000 cut max drawdown from -58k to -25k for ~2% of profit.

  D. MIN_MINUTES_LEFT_FOR_REENTRY
     Skip a re-entry when fewer than this many minutes remain before
     EXIT_TIME_IST (avoids 4 order charges for a 5-minute stub trade).

Recommended v3 config: straddle_config_DTE_1_v3.properties

---------------------------------------------------------------------------
v2 changes vs the original (all opt-in via the .properties file; leaving the new
keys unset reproduces the original P&L exactly):

  1. PROFIT_PROTECT_ARM_PCT / PROFIT_PROTECT_GIVEBACK_PCT
     The arm threshold and the trail give-back are no longer forced to be equal.

  2. BREAKEVEN_ARM_PCT / BREAKEVEN_LOCK_PCT
     Breakeven ratchet: a trade that ran into profit can no longer round-trip
     into a full stop-out. Validated by walk-forward: helped 5 of 6 time blocks,
     cut max drawdown ~30%, cut worst day from -22,945 to -16,561.

  3. REENTRY_MAX_PREMIUM_RATIO
     Stops re-entering when the ATM straddle is materially richer than the one
     just stopped out of (volatility still expanding).

  4. ALLOWED_DTE=0,1 is the recommended setting -- see FINDINGS.md. Trading the
     expiry day as well as the day before roughly doubles annual P&L WITHOUT
     increasing lots or concurrent exposure, because only one index is traded
     per day and attempts within a day are sequential.

NOTE: with the default config the Rs.3,000 stop cap binds on 100% of trades, so
LOSS_LIMIT_RUPEES_BY_ATTEMPT has no effect whatsoever. Do not spend time tuning
it unless you lower it below ~4.5% of premium.
"""
...
def _load_property_file() -> str:
    cfg_path = os.getenv(
        "STRADDLE_CONFIG",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "configs/straddle_config_DTE_1_v3_close_profit_optimized.properties"),
    )
...
# v2 ADDITIONS
# =============================================================================
# --- Profit-protect ARM and GIVEBACK are now separate ------------------------
# The original used ONE number for two different jobs: how much profit arms the
# trail, and how much of the peak you hand back before exiting. Leave either
# unset to fall back to PROFIT_PROTECT_TRIGGER_RUPEES (identical old behaviour).
_pp_arm_raw = os.getenv("PROFIT_PROTECT_ARM_PCT")
_pp_give_raw = os.getenv("PROFIT_PROTECT_GIVEBACK_PCT")
PROFIT_PROTECT_ARM_PCT = (_parse_pct_value(_pp_arm_raw) if _pp_arm_raw
                          else PROFIT_PROTECT_TRIGGER_RUPEES)
PROFIT_PROTECT_GIVEBACK_PCT = (_parse_pct_value(_pp_give_raw) if _pp_give_raw
                               else PROFIT_PROTECT_TRIGGER_RUPEES)

# --- Breakeven ratchet -------------------------------------------------------
# Once an attempt's peak profit reaches BREAKEVEN_ARM_PCT of the premium
# collected, the stop floor is lifted to BREAKEVEN_LOCK_PCT of premium, so a
# trade that ran into profit can no longer come back as a full stop-out.
#
# This is the only parameter change that survived walk-forward validation:
# it improved 5 of 6 independent time blocks and cut max drawdown ~30%.
# Set 0 to disable.
...
# Stop re-entering once the fresh ATM straddle is richer than the one just
# stopped out of by more than this ratio (volatility still expanding).
# 0 disables. 1.25 = skip if the new straddle costs >25% more.
REENTRY_MAX_PREMIUM_RATIO = _parse_float_env("REENTRY_MAX_PREMIUM_RATIO", 0.0)
...
# v3 ADDITIONS
# =============================================================================
# --- Re-entry after PROFIT_TARGET ------------------------------------------
# 1 = a PROFIT_TARGET exit no longer ends the day; the strategy re-enters at
# the (possibly new) ATM strike after REENTRY_DELAY_AFTER_TARGET minutes.
# 0 = original behaviour (target ends the day).
REENTRY_ON_PROFIT_TARGET = os.getenv("REENTRY_ON_PROFIT_TARGET", "0").strip() == "1"

# Separate per-attempt re-entry delay used ONLY after PROFIT_TARGET exits.
# Blank/unset -> fall back to REENTRY_DELAY_BY_ATTEMPT.
_rdat_raw = os.getenv("REENTRY_DELAY_AFTER_TARGET", "").strip()
REENTRY_DELAY_AFTER_TARGET = _parse_int_list(_rdat_raw, []) if _rdat_raw else []

def reentry_delay_after_target_for_attempt(attempt_idx: int) -> int:
    s = REENTRY_DELAY_AFTER_TARGET
    if not s:
        return reentry_delay_for_attempt(attempt_idx)
    return int(s[attempt_idx]) if attempt_idx < len(s) else int(s[-1])

# --- Day-level profit trail --------------------------------------------------
# Once cumulative realized NET P&L for the day reaches ARM rupees, the day is
# stopped as soon as realized P&L falls GIVEBACK rupees below the day's
# realized peak. GIVEBACK<=0 disables.
DAILY_PROFIT_TRAIL_ARM_RUPEES = _parse_float_env("DAILY_PROFIT_TRAIL_ARM_RUPEES", 0.0)
DAILY_PROFIT_TRAIL_GIVEBACK_RUPEES = _parse_float_env("DAILY_PROFIT_TRAIL_GIVEBACK_RUPEES", 0.0)

# --- Minimum minutes left for a re-entry ------------------------------------
# Skip a re-entry when fewer than this many minutes remain before
# EXIT_TIME_IST. 0 disables.
MIN_MINUTES_LEFT_FOR_REENTRY = int(float(os.getenv("MIN_MINUTES_LEFT_FOR_REENTRY", "0")))
...
_DEFAULT_OUT = os.path.join(
    ...
    f"short_straddle_backtest_v3_reattempt{_safe_fname_part(ENTRY_TIME_IST)}"
...
def simulate_day_multi_trades(
    *,
    ...
    profit_protect_enabled = float(PROFIT_PROTECT_ARM_PCT) > 0.0
    ...
    daily_realized_peak = 0.0            # v3: for the day-level profit trail
    daily_loss_limit_enabled = MAX_DAILY_LOSS_RUPEES > 0
    prev_entry_premium_per_unit = None   # v2: for REENTRY_MAX_PREMIUM_RATIO
    ...
    while cur_entry_ts < trade_end_ts:
        ...
        # --- v2: do not re-sell into still-expanding volatility ---
        if (trade_seq > 1 and REENTRY_MAX_PREMIUM_RATIO > 0
                and prev_entry_premium_per_unit):
            if (float(ce_entry) + float(pe_entry)) > (
                    prev_entry_premium_per_unit * REENTRY_MAX_PREMIUM_RATIO):
                skipped.append({
                    "day": dy, "underlying": und, "expiry": expiry,
                    "trade_seq": trade_seq, "atm_strike": atm,
                    "reason": (
                        f"No re-entry: ATM premium {float(ce_entry)+float(pe_entry):.2f} "
                        f"> {REENTRY_MAX_PREMIUM_RATIO:.2f}x previous "
                        f"{prev_entry_premium_per_unit:.2f}"),
                })
                break
        prev_entry_premium_per_unit = float(ce_entry) + float(pe_entry)
        ...
        # --- v2: breakeven ratchet ---------------------------------------
        # The stop floor starts at -loss_limit_rupees. Once the running peak of
        # the close-based P&L reaches BREAKEVEN_ARM_PCT of premium, the floor is
        # lifted to BREAKEVEN_LOCK_PCT of premium for the rest of the trade.
        stop_floor = pd.Series(-float(loss_limit_rupees), index=pnl_sl.index)
        if BREAKEVEN_ARM_PCT > 0:
            running_peak = pnl.cummax().reindex(pnl_sl.index).ffill()
            be_armed = running_peak >= (BREAKEVEN_ARM_PCT * entry_premium_sum)
            be_floor = float(BREAKEVEN_LOCK_PCT * entry_premium_sum)
            stop_floor = stop_floor.mask(be_armed.fillna(False),
                                         max(-float(loss_limit_rupees), be_floor))
        ...
        if profit_protect_enabled:
            peak = pnl.cummax()
            arm_rupees = float(PROFIT_PROTECT_ARM_PCT * entry_premium_sum)
            give_rupees = float(PROFIT_PROTECT_GIVEBACK_PCT * entry_premium_sum)
            armed = peak >= arm_rupees
            trail = peak - give_rupees
        ...
        if exit_reason == "STOPLOSS":
            # v2: with the ratchet armed this may be a small PROFIT, not a loss.
        ...
        daily_realized_peak = max(daily_realized_peak, daily_realized_pnl)  # v3
        ...
        # --- v3: day-level profit trail ---------------------------------
        if (DAILY_PROFIT_TRAIL_GIVEBACK_RUPEES > 0
                and daily_realized_peak >= DAILY_PROFIT_TRAIL_ARM_RUPEES
                and daily_realized_pnl <= daily_realized_peak - DAILY_PROFIT_TRAIL_GIVEBACK_RUPEES):
            skipped.append({
                "day": dy, "underlying": und, "expiry": expiry,
                "trade_seq": trade_seq + 1,
                "reason": (
                    f"No re-entry: day profit trail hit after trade_seq={trade_seq}; "
                    f"realized={daily_realized_pnl:.2f}, peak={daily_realized_peak:.2f}, "
                    f"giveback_limit={DAILY_PROFIT_TRAIL_GIVEBACK_RUPEES:.2f}"),
            })
            break

        _reenter_reasons = ("STOPLOSS", "PROFIT_PROTECT", "PROFIT_TARGET") \
            if REENTRY_ON_PROFIT_TARGET else ("STOPLOSS", "PROFIT_PROTECT")
        if exit_reason in _reenter_reasons and (trade_seq - 1) < MAX_REATTEMPTS:
            if exit_reason == "PROFIT_TARGET":
                delay_min = reentry_delay_after_target_for_attempt(trade_seq - 1)  # v3
            else:
            ...
            # v3: do not start a stub trade with almost no time left
            if (MIN_MINUTES_LEFT_FOR_REENTRY > 0
                    and (trade_end_ts - cur_entry_ts) < pd.Timedelta(minutes=MIN_MINUTES_LEFT_FOR_REENTRY)):
                skipped.append({
                    "day": dy, "underlying": und, "expiry": expiry,
                    "trade_seq": trade_seq,
                    "reason": (
                        f"No re-entry: fewer than {MIN_MINUTES_LEFT_FOR_REENTRY} minutes left "
                        f"before EXIT_TIME_IST {EXIT_TIME_IST}"),
                })
                break
...
def main():
    ...
    print(f"[INFO] v2 ProfitProtect arm/giveback: {PROFIT_PROTECT_ARM_PCT:.2%} / "
          f"{PROFIT_PROTECT_GIVEBACK_PCT:.2%}")
    print(f"[INFO] v2 Breakeven ratchet: arm {BREAKEVEN_ARM_PCT:.2%} -> lock "
          f"{BREAKEVEN_LOCK_PCT:.2%} of premium (0 = off)")
    print(f"[INFO] v2 Re-entry premium gate: {REENTRY_MAX_PREMIUM_RATIO:.2f}x (0 = off)")
    print(f"[INFO] v3 Re-entry on profit target: {REENTRY_ON_PROFIT_TARGET} | "
          f"delay after target: {REENTRY_DELAY_AFTER_TARGET or 'same as stop-loss ramp'}")
    print(f"[INFO] v3 Day profit trail: arm Rs {DAILY_PROFIT_TRAIL_ARM_RUPEES:.0f} / "
          f"giveback Rs {DAILY_PROFIT_TRAIL_GIVEBACK_RUPEES:.0f} (0 = off) | "
          f"min minutes left for re-entry: {MIN_MINUTES_LEFT_FOR_REENTRY}")
