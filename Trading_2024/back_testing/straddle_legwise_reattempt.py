...
# IMPORTANT CHANGE:
# Earlier logic:
#   stop_price = leg_entry_premium * (1 + LEG_STOP_PCT / 100)
#
# New logic:
#   stop_rise_amount = active_straddle_premium_sum * STRADDLE_SUM_STOP_PCT / 100
#   stop_price       = leg_entry_premium + stop_rise_amount
#
# Example:
#   CE entry = 100, PE entry = 120, premium sum = 220
#   STRADDLE_SUM_STOP_PCT = 30
#   stop rise amount = 220 * 30% = 66
#   CE stop = 100 + 66 = 166
#   PE stop = 120 + 66 = 186
#
# The leg whose option high touches/exceeds its stop price is exited.
STRADDLE_SUM_STOP_PCT = float(os.getenv("STRADDLE_SUM_STOP_PCT", "10"))

# Separate same-leg reattempt limits.
# Count increases only after the stopped leg is actually re-entered.
MAX_CE_REATTEMPTS = int(os.getenv("MAX_CE_REATTEMPTS", "4"))
MAX_PE_REATTEMPTS = int(os.getenv("MAX_PE_REATTEMPTS", "4"))

# No fixed-delay re-entry is used anywhere.
...
_DEFAULT_OUT = os.path.join(
    ...
    f"legwise_atm_short_straddle_sumstop_{_safe_fname_part(ENTRY_TIME_IST)}"
    f"_SSP_{_safe_fname_part(str(STRADDLE_SUM_STOP_PCT))}"
...
class ActiveLeg:
    ...
    # New stop fields:
    # stop_basis_premium_sum is the straddle premium sum used for stop calculation.
    # stop_rise_amount = stop_basis_premium_sum * STRADDLE_SUM_STOP_PCT / 100.
    # stop_price = entry_premium + stop_rise_amount.
    stop_basis_premium_sum: float
    stop_basis_source: str
    stop_rise_amount: float
    stop_price: float
...
def prepare_option_day_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    open is optional. If present, it allows conservative stop execution for
    gap-through-stop candles.
    """
    required_cols = [
        "date",
        "name",
...
class DayMarketContext:
    ...
    Option price series are built once per:
        (strike, option_type, symbol, column, ffill)

    This avoids repeated dataframe filtering in the minute-by-minute loop.
    """
    ...
    def _build_underlying_indexed(self, underlying_day: pd.DataFrame) -> pd.DataFrame:
        ...
        # Reindex to session minutes but do not forward-fill OHLC.
        # Reversal detection must use actual previous/current 1-minute candles.
...
# =============================================================================
# STRADDLE-SUM STOP BASIS HELPERS
# =============================================================================

def compute_stop_values_from_straddle_sum(
    *,
    leg_entry_premium: float,
    straddle_premium_sum: float,
) -> Tuple[float, float]:
    """
    New stop calculation.

    The leg is exited when its premium rises by a configured percentage of the
    straddle premium sum.

    stop_rise_amount = straddle_premium_sum * STRADDLE_SUM_STOP_PCT / 100
    stop_price       = leg_entry_premium + stop_rise_amount
    """
    stop_rise_amount = float(straddle_premium_sum) * float(STRADDLE_SUM_STOP_PCT) / 100.0
    stop_price = float(leg_entry_premium) + stop_rise_amount
    return round(stop_rise_amount, 4), round(stop_price, 4)


def assign_stop_basis_to_leg(
    *,
    leg: ActiveLeg,
    straddle_premium_sum: float,
    source: str,
    ...
    """
    Return updated ActiveLeg with stop based on straddle premium sum.
    """
    stop_rise_amount, stop_price = compute_stop_values_from_straddle_sum(
        leg_entry_premium=leg.entry_premium,
        straddle_premium_sum=straddle_premium_sum,
    )

    leg.stop_basis_premium_sum = float(straddle_premium_sum)
    leg.stop_basis_source = source
    leg.stop_rise_amount = float(stop_rise_amount)
    leg.stop_price = float(stop_price)

    return leg


def get_theoretical_opposite_atm_premium(
    *,
    ...
    """
    If a re-entered leg has no opposite active leg, this function attempts to
    price the opposite ATM option at the same strike/time.

    This keeps the stop basis as a 'straddle premium sum' even when only one
    leg is actually re-entered.

    Example:
      Re-enter CE, but no PE is open.
      Stop basis = CE entry premium + theoretical ATM PE close at re-entry time.
    """
    opposite_type = "PE" if option_type == "CE" else "CE"
    opposite_symbol = ctx.pick_symbol(strike, opposite_type)

    if not opposite_symbol:
        return None, float("nan")

    opposite_price = ctx.exact_option_price(
        strike=strike,
        opt_type=opposite_type,
    ...
    return opposite_symbol, opposite_price
...
def open_short_leg_without_stop_basis(
    *,
    und: str,
    dy: date,
    expiry: date,
    ...
    This function identifies symbol and entry premium, but stop basis is assigned
    separately because the new stop depends on straddle premium sum.
    """
    ...
    # Temporary placeholders for stop fields.
    # They are assigned immediately after stop basis premium sum is known.
    return ActiveLeg(
        day=dy,
        underlying=und,
        expiry=expiry,
        ...
        stop_basis_premium_sum=float("nan"),
        stop_basis_source="UNASSIGNED",
        stop_rise_amount=float("nan"),
...
def close_short_leg(
    *,
    ...
    return LegTradeRow(
        ...
        stop_basis_premium_sum=round(float(leg.stop_basis_premium_sum), 2),
        stop_basis_source=leg.stop_basis_source,
        stop_rise_amount=round(float(leg.stop_rise_amount), 2),
...
def simulate_day_legwise(
    *,
    ...
    State model:
      ...
      2. Initial CE and PE stop basis is initial CE premium + initial PE premium.
      3. Monitor each open leg independently using option candle high.
      4. A leg is stopped when it rises by STRADDLE_SUM_STOP_PCT of the relevant
         straddle premium sum.
      5. If CE stops, only CE exits and waits for bearish reversal.
      6. If PE stops, only PE exits and waits for bullish reversal.
      7. On reversal confirmation, re-enter only the same option type.
      8. ATM is recalculated at re-entry using latest underlying close.
      9. CE and PE may have different active strikes.
     10. All still-open legs are closed at SESSION_END_IST.
    """
    ...
    # -------------------------------------------------------------------------
    # Initial ATM short straddle.
    # Open CE and PE, then assign the same initial straddle premium sum basis
    # to both legs.
    # -------------------------------------------------------------------------
    initial_legs: Dict[str, ActiveLeg] = {}

    for opt_type in ["CE", "PE"]:
        leg_seq_counter += 1

        leg = open_short_leg_without_stop_basis(
            und=und,
            dy=dy,
            expiry=expiry,
            ctx=ctx,
        ...
        if leg is not None:
            initial_legs[opt_type] = leg

    if "CE" not in initial_legs or "PE" not in initial_legs:
        skipped.append(
            {
                "day": dy,
                "underlying": und,
                "expiry": expiry,
                "reason": "Initial ATM straddle could not be opened because CE or PE leg is missing",
            }
        )
        return trades, skipped

    initial_sum = initial_legs["CE"].entry_premium + initial_legs["PE"].entry_premium

    for opt_type, leg in initial_legs.items():
        active_legs[opt_type] = assign_stop_basis_to_leg(
            leg=leg,
            straddle_premium_sum=initial_sum,
            source="INITIAL_CE_ENTRY_PLUS_PE_ENTRY",
        )
    ...
    for ts in idx_all[start_pos + 1:]:
        ...
        for opt_type, leg in list(active_legs.items()):
            ...
            if not valid_price(leg.stop_price):
                skipped.append(
                    {
                        "day": dy,
                        "underlying": und,
                        ...
                        "reason": "Stop price invalid because straddle premium sum basis was not assigned",
                    }
                )
                continue
            ...
            if float(high_price) >= float(leg.stop_price):
                ...
                # - If high touches/exceeds stop_price, stop is considered hit.
                ...
                if valid_price(open_price) and float(open_price) > float(leg.stop_price):
                    ...
                    exit_reason = "LEG_STOP_GAP_OPEN_STRADDLE_SUM"
                else:
                    exit_premium = float(leg.stop_price)
                    exit_reason = "LEG_STOP_STRADDLE_SUM"
        ...
        for opt_type, pending in list(pending_reentries.items()):
            ...
            new_leg = open_short_leg_without_stop_basis(
                und=und,
                dy=dy,
                expiry=expiry,
                ctx=ctx,
            ...
            # -----------------------------------------------------------------
            # Assign straddle-sum stop basis to the re-entered leg.
            #
            # Case A: Opposite leg is open.
            #   Use:
            #       new leg entry premium + opposite open leg's current close
            #
            #   This represents the active premium sum of the currently combined
            #   position at re-entry time.
            #
            # Case B: No opposite leg is open.
            #   Use:
            #       new leg entry premium + theoretical opposite ATM premium
            #
            #   This preserves the requested 'straddle premium sum' basis even if
            #   only one leg is actually active after both earlier legs stopped.
            #
            # Existing open opposite leg is NOT retroactively re-based. Its stop
            # remains tied to the premium-sum basis assigned when that leg was
            # opened. This avoids changing risk rules mid-leg.
            # -----------------------------------------------------------------
            opposite_type = "PE" if opt_type == "CE" else "CE"
            stop_basis_source = ""

            if opposite_type in active_legs:
                opposite_leg = active_legs[opposite_type]
                opposite_current_premium = ctx.asof_option_price(
                    strike=opposite_leg.strike,
                    opt_type=opposite_leg.option_type,
                    symbol=opposite_leg.symbol,
                    price_col="close",
                    ts=ts,
                )

                if not valid_price(opposite_current_premium):
                    skipped.append(
                        {
                            "day": dy,
                            "underlying": und,
                            ...
                            "option_type": opt_type,
                            "event_time": ts.strftime("%H:%M"),
                            "opposite_option_type": opposite_type,
                            "reason": "Reversal found, but re-entry skipped because opposite active leg current premium is missing",
                        }
                    )
                    continue

                stop_basis_sum = float(new_leg.entry_premium) + float(opposite_current_premium)
                stop_basis_source = f"REENTRY_{opt_type}_ENTRY_PLUS_ACTIVE_{opposite_type}_CURRENT_PREMIUM"

            else:
                opposite_symbol, theoretical_opposite_premium = get_theoretical_opposite_atm_premium(
                    ctx=ctx,
                ...
                if not valid_price(theoretical_opposite_premium):
                    skipped.append(
                        {
                            "day": dy,
                            "underlying": und,
                            ...
                            "reason": "Reversal found, but re-entry skipped because theoretical opposite ATM premium is missing",
                        }
                    )
                    continue

                stop_basis_sum = float(new_leg.entry_premium) + float(theoretical_opposite_premium)
                stop_basis_source = f"REENTRY_{opt_type}_ENTRY_PLUS_THEORETICAL_ATM_{opposite_type}_PREMIUM"

            new_leg = assign_stop_basis_to_leg(
                leg=new_leg,
                straddle_premium_sum=stop_basis_sum,
                source=stop_basis_source,
            )
...
def write_excel(
    ...
    config_df = pd.DataFrame(
        [
            ...
            {"parameter": "STRADDLE_SUM_STOP_PCT", "value": STRADDLE_SUM_STOP_PCT},
            ...
            {
                "parameter": "STOP_DETECTION",
                "value": "Exit leg if option high >= leg_entry_premium + STRADDLE_SUM_STOP_PCT% of straddle premium sum",
            },
            {
                "parameter": "STOP_EXECUTION",
                "value": "If option open > stop_price, exit at open; otherwise exit at stop_price",
            },
            {
                "parameter": "REENTRY_LOGIC",
                "value": "No fixed delay; same-leg re-entry only after full-cover underlying reversal candle",
            },
...
def main() -> None:
    ...
    print(
        ...
        f"StraddleSumStopPct={STRADDLE_SUM_STOP_PCT} | "
