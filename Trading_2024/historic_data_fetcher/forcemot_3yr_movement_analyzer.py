...
Output Excel sheets:
    ...
    3. yearly_summary
    4. weekday_summary
    5. month_range_distribution
    6. month_body_distribution
    7. config
...
Kite sometimes returns timezone-aware dates like:

    2026-06-09 00:00:00+05:30

This version fixes that by:
    1. normalizing all Kite timestamps to timezone-naive IST;
    2. applying make_excel_safe_df() before writing every Excel sheet.
...
def to_ist_naive_timestamp(value: Any) -> pd.Timestamp:
    """
    Convert any Kite/Pandas/Python datetime into timezone-naive IST Timestamp.
...
def make_excel_safe_df(df: pd.DataFrame) -> pd.DataFrame:
    """
    Make a dataframe safe for Excel writing.

    Excel cannot write timezone-aware datetimes. This function:
        - converts timezone-aware datetime columns to timezone-naive;
        - converts object columns containing Timestamps/dates safely;
    ...
    for col in out.columns:
        ...
        # Direct pandas datetime columns.
        if pd.api.types.is_datetime64_any_dtype(series):
            try:
                if getattr(series.dt, "tz", None) is not None:
                    out[col] = series.dt.tz_convert("Asia/Kolkata").dt.tz_localize(None)
                else:
                    out[col] = pd.to_datetime(series, errors="coerce")
            except Exception:
                out[col] = pd.to_datetime(series.astype(str), errors="coerce")

        # Object columns may contain Python date/datetime/Timestamp values.
        elif series.dtype == "object":
            sample = series.dropna().head(10).tolist()

            if any(isinstance(x, (datetime, date, pd.Timestamp)) for x in sample):
                converted = [to_ist_naive_timestamp(x) for x in series.tolist()]
                out[col] = pd.to_datetime(converted, errors="coerce")

    return out


def compute_date_range() -> Tuple[date, date]:
    """Compute download date range with small buffer."""
    to_date = ist_today()
...
def initialize_kite():
    """Initialize Kite using your existing helper."""
    if oUtils is None:
        raise RuntimeError(
...
def resolve_equity_token(kite, symbol: str, exchange: str = EXCHANGE) -> InstrumentInfo:
    """Resolve NSE equity instrument token."""
...
def load_cached_if_usable(symbol: str, from_date: date, to_date: date) -> Optional[pd.DataFrame]:
    """Load cache if it covers requested date range."""
    if FORCE_REFRESH:
        return None
...
def rows_to_dataframe(rows: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    # Critical fix: normalize timestamps to timezone-naive IST.
...
def add_daily_movement_features(df: pd.DataFrame, symbol: str, instrument_token: int) -> pd.DataFrame:
    """Add detailed daily movement features."""
    if df.empty:
        raise ValueError("No data supplied to add_daily_movement_features().")

    out = df.copy()

    # Make all date columns Excel-safe at source.
    out["date"] = pd.to_datetime(
        [to_ist_naive_timestamp(x) for x in out["date"].tolist()],
        errors="coerce",
    )
    ...
    out = out.dropna(subset=["date", "trade_date"]).copy()
    ...
    out["gap_direction"] = np.select(
        [out["gap_pct"] > 0.10, out["gap_pct"] < -0.10],
        ["GAP_UP", "GAP_DOWN"],
        default="FLAT_OPEN",
    )
    ...
    out["day_type"] = np.select(
        [out["body_signed_pct"] > 0.10, out["body_signed_pct"] < -0.10],
        ["GREEN", "RED"],
...
def make_yearly_summary(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    for yr, g in daily.groupby("year", sort=True):
        g = g.sort_values("trade_date").copy()
...
def make_month_bucket_distribution(daily: pd.DataFrame, bucket_col: str) -> pd.DataFrame:
    """Create monthly distribution for range/body buckets."""
    if daily.empty:
        return pd.DataFrame()
...
def add_excel_charts(writer: pd.ExcelWriter, month_summary: pd.DataFrame) -> None:
    """Add simple monthly charts."""
    if month_summary.empty:
        return

    workbook = writer.book
    ws = writer.sheets.get("month_summary")
...
def write_excel_report(
    ...
    """Write all analysis sheets to Excel safely."""
    # Critical Excel-safety conversion.
    daily = make_excel_safe_df(daily)
    month_summary = make_excel_safe_df(month_summary)
    yearly_summary = make_excel_safe_df(yearly_summary)
    weekday_summary = make_excel_safe_df(weekday_summary)
    month_range_distribution = make_excel_safe_df(month_range_distribution)
    month_body_distribution = make_excel_safe_df(month_body_distribution)
    ...
    with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
        ...
        for sheet_name, df in sheets.items():
            ...
            for col_name in df.columns:
                ...
                elif "days" in lower or lower == "volume":
...
def main() -> None:
    ...
    raw_df = raw_df.dropna(subset=["trade_date"]).copy()
    ...
    config_df = pd.DataFrame(
        [
            ...
            {"parameter": "EXCEL_TIMEZONE_FIX", "value": "All datetime columns converted to timezone-naive before to_excel()."},
        ]
    )
