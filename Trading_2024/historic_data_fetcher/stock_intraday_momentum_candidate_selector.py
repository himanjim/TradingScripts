...
INPUT_CSV = os.environ.get(
    "INPUT_CSV",
    r"C:\Users\Local User\Downloads\fo_mktlots.csv",
).strip()
...
def add_daily_features(symbol: str, instrument_token: int, raw_df: pd.DataFrame) -> pd.DataFrame:
    """Add daily range and approximate traded-value features."""
    if raw_df.empty:
    ...
    df["daily_range_abs"] = df["high"] - df["low"]
    df["daily_range_pct"] = (df["daily_range_abs"] / df["close"]) * 100.0
    ...
    # Remove impossible rows. A 25% high-low day is possible in rare cases, but
    # for a systematic selector it is safer to exclude obvious bad data spikes.
    df = df[
        (df["close"] > 0)
...
def main() -> None:
    """Script entrypoint."""
    print("============================================================")
    print("STOCK INTRADAY MOMENTUM CANDIDATE SELECTOR")
    print("============================================================")
    ...
    config_df = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "value": (
                    "30% 1Y median range; 20% 2Y median range; 15% 3M median range; "
