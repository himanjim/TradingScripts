"""
ATM Short Straddle Backtest (with Virtual Monitor Trigger + Reattempts)

What this script does
---------------------
For each trading day and each underlying (NIFTY, SENSEX) it:
  1) Identifies the nearest expiry for that day from your option pickle files (PASS-1).
  2) Downloads 1-minute underlying OHLCV from Kite for a configurable window (LOOKBACK_MONTHS).
  3) For each (underlying, day, nearest-expiry) group in the pickles (PASS-2), it runs:

     A) Virtual monitoring phase (no actual trade yet)
        - Start time: MONITOR_START_IST (default 09:20 IST).
        - Define a *virtual* ATM short straddle at monitor-start:
            * ATM strike is computed from underlying close at monitor-start.
            * Virtual entry price = CE_close + PE_close at monitor-start (same strike).
        - Track virtual PnL minute-by-minute (short CE + short PE).
        - Take the first *actual* trade only when virtual PnL hits either:
            * loss <= -MONITOR_TRIGGER_LOSS_RUPEES, OR
            * profit >=  MONITOR_TRIGGER_PROFIT_RUPEES
          If both triggers are disabled, the first actual trade is taken immediately at monitor-start.

     B) Actual trading phase (real backtest trade rows)
        - At the trigger timestamp, recompute ATM using underlying close *at that minute* and short ATM CE+PE.
        - Manage exits using:
            * STOPLOSS: pnl <= -LOSS_LIMIT_RUPEES
            * PROFIT_PROTECT: trailing exit after peak pnl reaches PROFIT_PROTECT_TRIGGER_RUPEES
              (current implementation uses a symmetric 'giveback' equal to the trigger)
            * Else EOD exit at SESSION_END_IST
        - If exited via STOPLOSS or PROFIT_PROTECT, re-enter after REENTRY_DELAY_MINUTES,
          up to MAX_REATTEMPTS times.

Inputs expected
---------------
Option pickles in PICKLES_DIR must contain at least:
  date, name, type, option_type, strike, expiry, instrument, close

How to run
----------
- Ensure Trading_2024.OptionTradeUtils is on PYTHONPATH and your Kite credentials are set.
- Optional environment variables:
    PICKLES_DIR, OUTPUT_XLSX, LOOKBACK_MONTHS,
    MONITOR_START_IST, MONITOR_TRIGGER_LOSS_RUPEES, MONITOR_TRIGGER_PROFIT_RUPEES,
    LOSS_LIMIT_RUPEES, PROFIT_PROTECT_TRIGGER_RUPEES, MAX_REATTEMPTS, REENTRY_DELAY_MINUTES,
    FAIL_ON_PICKLE_ERROR.

Notes
-----
- All timestamps are normalized to Asia/Kolkata (IST).
- Virtual monitoring uses a *fixed* strike based on monitor-start ATM (not rolling ATM).
"""
...
# USER CONFIG
# =============================================================================
# Environment variable overrides are supported for most knobs.
# If you run on Windows, prefer raw strings (r"C:\\path") when setting OUTPUT_XLSX.

PICKLES_DIR = os.getenv("PICKLES_DIR", r"G:\My Drive\Trading\Historical_Options_Data")  # <-- change or set env PICKLES_DIR            # <-- change
ENTRY_TIME_IST = os.getenv("ENTRY_TIME_IST", "09:20")  # "HH:MM" (backward-compat; used as monitor start if MONITOR_START_IST not set)
MONITOR_START_IST = os.getenv("MONITOR_START_IST", ENTRY_TIME_IST)  # "HH:MM"
MONITOR_TRIGGER_LOSS_RUPEES_ENV = os.getenv("MONITOR_TRIGGER_LOSS_RUPEES", "5000")  # None/off to disable
MONITOR_TRIGGER_PROFIT_RUPEES_ENV = os.getenv("MONITOR_TRIGGER_PROFIT_RUPEES", "5000")  # None/off to disable
...
_DEFAULT_OUT = rf"C:\Users\himan\Downloads\short_straddle_backtest_monitor_virtual_{_safe_fname_part(MONITOR_START_IST)}_MTL_{_safe_fname_part(str(MONITOR_TRIGGER_LOSS_RUPEES_ENV))}_MTP_{_safe_fname_part(str(MONITOR_TRIGGER_PROFIT_RUPEES_ENV))}_LL_{_safe_fname_part(str(LOSS_LIMIT_RUPEES))}_PPT_{_safe_fname_part(str(PROFIT_PROTECT_TRIGGER_RUPEES))}_RDM_{_safe_fname_part(str(REENTRY_DELAY_MINUTES))}_MR_{_safe_fname_part(str(MAX_REATTEMPTS))}.xlsx"
...
def _parse_optional_pos_int(s: str) -> Optional[int]:
    """
    Parse an env-var style integer that can also disable a feature.

    Accepted "disabled" values (case-insensitive): "", none, null, off, disable, disabled
    Returns:
      - None  -> feature disabled
      - int>0 -> feature enabled with that positive value
    """
    if s is None:
        return None
    t = str(s).strip().lower()
    if t in ("", "none", "null", "off", "disable", "disabled"):
        return None
    try:
        v = int(t)
    except Exception as e:
        raise ValueError(f"Expected a positive integer or None/off, got: {s!r}") from e
    if v <= 0:
        # Treat 0/negative as disabled rather than silently misbehaving
        return None
    return v

MONITOR_START_TIME = parse_hhmm(MONITOR_START_IST)
MONITOR_TRIGGER_LOSS_RUPEES = _parse_optional_pos_int(MONITOR_TRIGGER_LOSS_RUPEES_ENV)
MONITOR_TRIGGER_PROFIT_RUPEES = _parse_optional_pos_int(MONITOR_TRIGGER_PROFIT_RUPEES_ENV)
...
class TradeRow:
    ...
    monitor_atm_strike: int
    monitor_trigger: str
    monitor_trigger_time: str
    monitor_virtual_pnl_at_trigger: float

# =============================================================================
# PASS-1: determine nearest expiry per (underlying, day)
#         This allows us to consistently backtest the 'front expiry' straddle for each day.
# =============================================================================
...
def simulate_day_multi_trades(
    *,
    ...
    # Cache option leg minute-series within this (underlying, day, expiry) run.
    # This avoids rebuilding/reindexing the same CE/PE series repeatedly across re-attempts.
    leg_cache: Dict[Tuple[int, str, str], pd.Series] = {}

    def _leg_series_cached(strike: int, opt_type: str, symbol: str) -> pd.Series:
        key = (int(strike), str(opt_type).upper(), str(symbol))
        if key not in leg_cache:
            leg_cache[key] = _build_leg_series(day_opt, idx_all, int(strike), str(opt_type).upper(), str(symbol))
    ...
    # -------------------------------------------------------------------------
    # 0) "Virtual monitoring" of an ATM short straddle from MONITOR_START_TIME.
    #    First actual trade is taken only when virtual PnL hits either:
    #      - loss <= -MONITOR_TRIGGER_LOSS_RUPEES, or
    #      - profit >= MONITOR_TRIGGER_PROFIT_RUPEES
    #    If both triggers are disabled (None/off), entry is immediate at monitor start.
    # -------------------------------------------------------------------------
    monitor_start_ts = pd.Timestamp(datetime.combine(dy, MONITOR_START_TIME), tz=ist_tz())
    if monitor_start_ts < session_start_ts:
        monitor_start_ts = session_start_ts
    if monitor_start_ts > session_end_ts:
        skipped.append({
            "day": dy, "underlying": und, "expiry": expiry,
            "reason": f"Monitor start {MONITOR_START_TIME.strftime('%H:%M')} beyond session end"
        })
        return results, skipped

    u_px_m = asof_close(underlying_day, monitor_start_ts)
    if pd.isna(u_px_m):
        skipped.append({"day": dy, "underlying": und, "expiry": expiry,
                        "reason": f"No underlying price at monitor start {monitor_start_ts.strftime('%H:%M')}"})
        return results, skipped

    monitor_atm = round_to_step(float(u_px_m), step)

    m_ce_sym = _pick_symbol(day_opt, monitor_atm, "CE")
    m_pe_sym = _pick_symbol(day_opt, monitor_atm, "PE")
    if not m_ce_sym or not m_pe_sym:
        skipped.append({"day": dy, "underlying": und, "expiry": expiry,
                        "atm_strike": monitor_atm,
                        "reason": "Monitor ATM CE/PE not available in pickle band"})
        return results, skipped

    m_ce_s = _leg_series_cached(monitor_atm, "CE", m_ce_sym)
    m_pe_s = _leg_series_cached(monitor_atm, "PE", m_pe_sym)

    m_ce_entry = m_ce_s.loc[monitor_start_ts]
    m_pe_entry = m_pe_s.loc[monitor_start_ts]
    if pd.isna(m_ce_entry) or pd.isna(m_pe_entry):
        skipped.append({"day": dy, "underlying": und, "expiry": expiry,
                        "atm_strike": monitor_atm,
                        "reason": "No CE/PE price at monitor start (after ffill)"})
        return results, skipped

    virt_pnl_all = (float(m_ce_entry) - m_ce_s) * qty + (float(m_pe_entry) - m_pe_s) * qty
    virt_pnl = virt_pnl_all.loc[monitor_start_ts:].dropna()
    if virt_pnl.empty:
        skipped.append({"day": dy, "underlying": und, "expiry": expiry,
                        "atm_strike": monitor_atm,
                        "reason": "Virtual PnL series empty after monitor start"})
        return results, skipped

    trigger_ts: Optional[pd.Timestamp] = None
    trigger_kind = "IMMEDIATE"

    loss_thr = MONITOR_TRIGGER_LOSS_RUPEES
    profit_thr = MONITOR_TRIGGER_PROFIT_RUPEES

    if (loss_thr is None) and (profit_thr is None):
        trigger_ts = monitor_start_ts
        trigger_kind = "IMMEDIATE"
    else:
        # Find the first minute where virtual PnL breaches either threshold.
        # We compute both (if enabled) and pick the earlier timestamp.
        loss_ts = None
        profit_ts = None

        if loss_thr is not None:
            hit = (virt_pnl <= -float(loss_thr))
            if hit.any():
                loss_ts = virt_pnl.index[hit.to_numpy().argmax()]

        if profit_thr is not None:
            hit = (virt_pnl >= float(profit_thr))
            if hit.any():
                profit_ts = virt_pnl.index[hit.to_numpy().argmax()]

        if loss_ts is not None and profit_ts is not None:
            if loss_ts < profit_ts:
                trigger_ts, trigger_kind = loss_ts, "VIRT_LOSS"
            elif profit_ts < loss_ts:
                trigger_ts, trigger_kind = profit_ts, "VIRT_PROFIT"
            else:
                trigger_ts, trigger_kind = loss_ts, "VIRT_LOSS"
        elif loss_ts is not None:
            trigger_ts, trigger_kind = loss_ts, "VIRT_LOSS"
        elif profit_ts is not None:
            trigger_ts, trigger_kind = profit_ts, "VIRT_PROFIT"
        else:
            trigger_ts = None
    ...
    if trigger_ts is None:
        skipped.append({
            "day": dy, "underlying": und, "expiry": expiry,
            "atm_strike": monitor_atm,
            "reason": f"Monitor trigger not hit (loss<=-{loss_thr} or profit>={profit_thr})"
        })
        return results, skipped

    virt_pnl_at_trigger = float(virt_pnl.loc[trigger_ts]) if trigger_ts in virt_pnl.index else float("nan")

    # -------------------------------------------------------------------------
    # 1) Actual trades start at trigger_ts; reattempt logic unchanged.
    # -------------------------------------------------------------------------
    cur_entry_ts = trigger_ts
    trade_seq = 1

    reentry_delay = max(1, int(REENTRY_DELAY_MINUTES))
    ...
    while cur_entry_ts <= session_end_ts:
        ...
        ce_s = _leg_series_cached(atm, "CE", ce_sym)
        pe_s = _leg_series_cached(atm, "PE", pe_sym)
        ...
        # STOPLOSS: exit as soon as PnL breaches the loss limit.
        stop_hit = pnl <= -LOSS_LIMIT_RUPEES
        stop_ts = pnl.index[stop_hit.to_numpy().argmax()] if stop_hit.any() else None

        # PROFIT_PROTECT: once peak PnL reaches threshold G, set a trailing stop at (peak - G).
        # Note: This uses the same value G for both 'arm threshold' and 'giveback'.
        protect_ts = None
        ...
        results.append(
            TradeRow(
                ...
                max_loss=max_loss,
                monitor_start_time=pd.Timestamp(monitor_start_ts).strftime("%H:%M"),
                monitor_atm_strike=int(monitor_atm),
                monitor_trigger=str(trigger_kind),
                monitor_trigger_time=pd.Timestamp(trigger_ts).strftime("%H:%M"),
                monitor_virtual_pnl_at_trigger=float(virt_pnl_at_trigger),
            )
        )

        if exit_reason in ("STOPLOSS", "PROFIT_PROTECT") and (trade_seq - 1) < MAX_REATTEMPTS:
            trade_seq += 1
            cur_entry_ts = pd.Timestamp(exit_ts) + pd.Timedelta(minutes=reentry_delay)
...
# =============================================================================
# PASS-2: process each pickle and simulate trades
#         For each (underlying, day, expiry) that matches PASS-1 nearest expiry, run the backtest.
#         We deduplicate across multiple pickles to avoid double-counting the same day.
# =============================================================================
def process_pickles_generate_trades(
    ...
    processed_day_keys = set()  # type: set[Tuple[str, date, date]]
...
def write_excel(all_trades_df: pd.DataFrame, actual_trades_df: pd.DataFrame, skipped_df: pd.DataFrame) -> None:
    ...
    # Pivots are useful for quickly comparing per-day PnL across underlyings.
...
def main():
    ...
    print(f"[INFO] Monitor start: {MONITOR_START_IST} | Monitor triggers: loss<=-{MONITOR_TRIGGER_LOSS_RUPEES} profit>={MONITOR_TRIGGER_PROFIT_RUPEES}")
    print(f"[INFO] Stoploss: -{LOSS_LIMIT_RUPEES} | ProfitProtect giveback: {PROFIT_PROTECT_TRIGGER_RUPEES} | Re-entry delay min: {REENTRY_DELAY_MINUTES} | Max reattempts: {MAX_REATTEMPTS}")
