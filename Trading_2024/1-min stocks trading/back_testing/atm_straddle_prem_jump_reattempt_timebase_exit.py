...
_max_lat_env = os.getenv("MAX_LOSS_AFTER_TIME_REATTEMPTS", "1").strip()
if _max_lat_env == "":
    _max_lat_env = "1"
try:
    MAX_LOSS_AFTER_TIME_REATTEMPTS = int(_max_lat_env)
    if MAX_LOSS_AFTER_TIME_REATTEMPTS < 0:
        raise ValueError("negative")
except Exception as e:
    raise ValueError(f"MAX_LOSS_AFTER_TIME_REATTEMPTS must be a non-negative integer; got: {_max_lat_env!r}") from e
...
# Optional timed-loss exit:
# If LOSS_EXIT_AFTER_MINUTES is set to an integer N, then for each trade attempt we will check
# the combined straddle PnL at (entry_time + N minutes). If the PnL is still negative then,
# we exit immediately at that timestamp (reason: LOSS_AFTER_TIME). If set to "None" / unset,
# this feature is disabled.
_loss_exit_env = os.getenv("LOSS_EXIT_AFTER_MINUTES", "60").strip()
if _loss_exit_env == "" or _loss_exit_env.lower() in ("none", "null", "off", "disable", "disabled"):
    LOSS_EXIT_AFTER_MINUTES: Optional[int] = None
else:
    try:
        LOSS_EXIT_AFTER_MINUTES = int(_loss_exit_env)
        if LOSS_EXIT_AFTER_MINUTES < 0:
            raise ValueError("negative")
    except Exception as e:
        raise ValueError(f"LOSS_EXIT_AFTER_MINUTES must be an integer minutes (e.g., 20) or None to disable; got: {_loss_exit_env!r}") from e

_LAM_TAG = "OFF" if LOSS_EXIT_AFTER_MINUTES is None else str(LOSS_EXIT_AFTER_MINUTES)
_DEFAULT_OUT = rf"C:\Users\himan\Downloads\short_straddle_backtest_reattempt_{_safe_fname_part(ENTRY_TIME_IST)}_LL_{LOSS_LIMIT_RUPEES}_PPT_{PROFIT_PROTECT_TRIGGER_RUPEES}_RDM_{REENTRY_DELAY_MINUTES}_LAM_{_safe_fname_part(_LAM_TAG)}_MR_{MAX_REATTEMPTS}_MLAT_{MAX_LOSS_AFTER_TIME_REATTEMPTS}_LE_{LOSS_EXIT_AFTER_MINUTES}.xlsx"
...
def simulate_day_multi_trades(
    *,
    ...
    # Cache per-leg 1-minute series to avoid rebuilding the same symbol series across re-entries.
    # Keyed by (strike, option_type, symbol).
    ...
    # Re-entry counters (kept separate so different exit reasons can have independent limits).
    # - risk_reattempts_used: re-entries used after STOPLOSS / PROFIT_PROTECT exits
    # - timeloss_reattempts_used: re-entries used after LOSS_AFTER_TIME exits
    risk_reattempts_used = 0
    timeloss_reattempts_used = 0
    while cur_entry_ts <= session_end_ts:
        ...
        ce_key = (atm, "CE", str(ce_sym))
        if ce_key not in leg_cache:
            leg_cache[ce_key] = _build_leg_series(day_opt, idx_all, atm, "CE", ce_sym)
        ce_s = leg_cache[ce_key]

        pe_key = (atm, "PE", str(pe_sym))
        if pe_key not in leg_cache:
            leg_cache[pe_key] = _build_leg_series(day_opt, idx_all, atm, "PE", pe_sym)
        pe_s = leg_cache[pe_key]
        ...
        # Optional timed-loss exit: check PnL at (entry + N minutes) and exit if still negative.
        time_loss_ts = None
        if LOSS_EXIT_AFTER_MINUTES is not None:
            chk = pd.Timestamp(cur_entry_ts) + pd.Timedelta(minutes=int(LOSS_EXIT_AFTER_MINUTES))
            if chk <= eod_ts:
                # Ensure chk aligns to an available minute in the PnL index (pad to previous minute if needed).
                if chk not in pnl.index:
                    loc2 = pnl.index.get_indexer([chk], method="pad")
                    chk = pnl.index[loc2[0]] if loc2[0] != -1 else None
                if chk is not None:
                    try:
                        if float(pnl.loc[chk]) < 0.0:
                            time_loss_ts = chk
                    except Exception:
                        time_loss_ts = None

        # Choose the earliest exit among: STOPLOSS / PROFIT_PROTECT / LOSS_AFTER_TIME / EOD.
        candidates: List[Tuple[pd.Timestamp, str]] = [(eod_ts, "EOD")]
        if stop_ts is not None:
            candidates.append((stop_ts, "STOPLOSS"))
        if protect_ts is not None:
            candidates.append((protect_ts, "PROFIT_PROTECT"))
        if time_loss_ts is not None:
            candidates.append((time_loss_ts, "LOSS_AFTER_TIME"))

        _prio = {"STOPLOSS": 0, "PROFIT_PROTECT": 1, "LOSS_AFTER_TIME": 2, "EOD": 9}
        exit_ts, exit_reason = min(candidates, key=lambda t: (t[0], _prio.get(t[1], 5)))
        ...
        # Decide if we should place a follow-up trade (re-entry).
        # NOTE: We keep separate budgets:
        #   - MAX_REATTEMPTS applies only to STOPLOSS / PROFIT_PROTECT exits.
        #   - MAX_LOSS_AFTER_TIME_REATTEMPTS applies only to LOSS_AFTER_TIME exits.
        should_reenter = False
        if exit_reason in ("STOPLOSS", "PROFIT_PROTECT"):
            if risk_reattempts_used < MAX_REATTEMPTS:
                risk_reattempts_used += 1
                should_reenter = True
        elif exit_reason == "LOSS_AFTER_TIME":
            if timeloss_reattempts_used < MAX_LOSS_AFTER_TIME_REATTEMPTS:
                timeloss_reattempts_used += 1
                should_reenter = True

        if should_reenter:
            trade_seq += 1
            cur_entry_ts = pd.Timestamp(exit_ts) + pd.Timedelta(minutes=max(1, int(REENTRY_DELAY_MINUTES)))
...
def write_excel(all_trades_df: pd.DataFrame, actual_trades_df: pd.DataFrame, skipped_df: pd.DataFrame) -> None:
    ...
    if not all_trades_df.empty:
        ...
        inst["is_loss_after_time"] = inst["exit_reason"].astype(str).str.upper().eq("LOSS_AFTER_TIME")
        instrument_summary = (
            ...
            .agg(
                ...
                loss_after_time_rate_pct=("is_loss_after_time", lambda s: 100.0 * s.mean()),
...
def main():
    ...
    print(f"[INFO] Stoploss: -{LOSS_LIMIT_RUPEES} | ProfitProtect giveback: {PROFIT_PROTECT_TRIGGER_RUPEES} | Re-entry delay min: {REENTRY_DELAY_MINUTES} | LossAfterMin: {LOSS_EXIT_AFTER_MINUTES if LOSS_EXIT_AFTER_MINUTES is not None else 'OFF'} | LossAfterTimeReentries: {MAX_LOSS_AFTER_TIME_REATTEMPTS}")
