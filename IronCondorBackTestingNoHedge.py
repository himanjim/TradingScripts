    ...
    for trading_date_ in trading_dates_:
        ...
        if (sell_call_entry_price_ + sell_put_entry_price_ - sell_call_high_price - sell_put_low_price) < maximum_beareable_loss:
            ...
            if trading_date_ == expiry_date_str_:
                under_lying_value_orig_new_= under_lying_value_orig_ + ((sell_put_entry_price + sell_call_entry_price - maximum_beareable_loss) * .9)
...
if __name__ == '__main__':
    ...
    for expiry_date_str in expiry_dates:
        ...
        if trading_date_obj is not None:
            ...
            if managed_profit < 0:
                unavoidable_loss, under_lying_value_strangle, strangle_call_open_price, strangle_call_exit_price, strangle_put_open_price, strangle_put_exit_price, strangle_p_l, full_strangle_p_l, second_trading_date = find_second_trade_pl(df, sell_call_strike, sell_call_entry_price, sell_put_strike, sell_put_entry_price, trading_date_obj, expiry_date_str, strike_difference, under_lying_value)
            else:
                unavoidable_loss, under_lying_value_strangle, strangle_call_open_price, strangle_call_exit_price, strangle_put_open_price, strangle_put_exit_price, second_trading_date = [None] * 7
                strangle_p_l = 0
                full_strangle_p_l = 0
    ...
    if len(trading_outputs) > 0:
        ...
        print("Accuracy(P/L):", round(excel_df['PROFIT'].sum() / excel_df.shape[0], 1))
        ...
        print("Accuracy(MGD PROFIT):", round(excel_df['MGD P/L'].sum() / excel_df.shape[0], 1))

        print("\n STRANGLE PROFIT:", excel_df['STRANGLE P/L'].sum())
        print("Accuracy(STRANGLE PROFIT):", round(excel_df['STRANGLE SELL PROFIT'].sum() / len(excel_df[excel_df['MGD P/L'] == 0]), 1))
        ...
        print("Accuracy(FULL STRANGLE PROFIT):", round(excel_df['FULL STRANGLE SELL PROFIT'].sum() / len(excel_df[excel_df['MGD P/L'] == 0]), 1))
