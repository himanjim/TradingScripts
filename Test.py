...
def string_to_datetime(datetime_str_):
    datetime_str_ = datetime_str_.replace('+0530', '')

    # Parse the string into a datetime object
    return datetime.strptime(datetime_str_, '%Y-%m-%dT%H:%M:%S')


def read_json(json_file_):
    with open(json_file_, 'r') as file:
        json_data = json.load(file)

    # Extract the values of the "candles" key
    candles = json_data['data']['candles']

    # Define the column names
    columns = ["Date", "Open", "High", "Low", "Close", "Volume", "Unknown"]

    # Convert the extracted values into a pandas DataFrame
    return pd.DataFrame(candles, columns=columns)


if __name__ == '__main__':
    json_files = glob.glob("D:/BN 5 min/*.txt")

    with ThreadPoolExecutor() as executor:
        dfs_ = list(executor.map(read_json, json_files))

    # Concatenate all DataFrames into a single DataFrame
    full_pd = pd.concat(dfs_, ignore_index=True)

    full_pd['Date'] = full_pd['Date'].apply(string_to_datetime)

    full_pd = full_pd.sort_values(by='Date')

    full_pd.to_pickle('D:/BN 5 min/BN_5_min.pkl')

    full_pd['Date_Only'] = full_pd['Date'].dt.date
    full_pd['Time_Only'] = full_pd['Date'].dt.time

    date_time_high_low_map = {}

    for index, row in full_pd.iterrows():
        date_only = row['Date_Only']
        time_only = row['Time_Only']

        if date_only in date_time_high_low_map:
            value = date_time_high_low_map[date_only]['Value']
            open_ = date_time_high_low_map[date_only]['Open']

            if abs(open_ - row['High']) > abs(open_ - value) and abs(open_ - row['High']) > abs(open_ - row['Low']):
                date_time_high_low_map[date_only]['Value'] = row['High']
                date_time_high_low_map[date_only]['Time'] = time_only

            elif abs(open_ - row['Low']) > abs(open_ - value) and abs(open_ - row['Low']) > abs(open_ - row['High']):
                date_time_high_low_map[date_only]['Value'] = row['Low']
                date_time_high_low_map[date_only]['Time'] = time_only

        else:

            date_time_high_low_map[date_only] = {'Open': row['Open'], 'Time': time_only, 'Value': None}
    ...
    print(date_time_high_low_map)

    df = pd.DataFrame(date_time_high_low_map)
